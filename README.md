...
PIN values are stored in plaintext in the database; in production I would hash them with bcrypt so a database breach doesn't expose credentials directly. I would also introduce a second authentication factor — for example, verifying the customer's date of birth alongside their PIN — to make impersonation significantly harder. Issued tokens currently live forever in an in-memory dictionary, so I would add expiry timestamps and move the store to Redis or a database table to survive server restarts. I would replace the hand-rolled hex token with a signed JWT so the token itself carries a verifiable expiry claim and the server doesn't need to store state at all. I would also add per-account rate limiting on the authentication endpoint to prevent brute-force guessing, and swap SQLite for a proper hosted database such as Postgres so the data layer is production-grade, independently managed, and doesn't reset on redeployment.
